...
//		}
		...
		return group
				.resultMessage(JSONHelper.string2json(group.AddGroup(ginfos)));
	}
...
//		}
		...
		String tips = execRequest._run("GrapeContent/content/SetGroupBatch/s:"
	...
	public String FindByType(String type, int no) {
		return group.resultMessage(group.findByType(type, no));
	}
...
//		}
	...
	public String GroupPage(int idx, int pageSize) {
		return group.resultMessage(group.page(idx, pageSize));
	}

	// 条件分页
	public String GroupPageBy(int idx, int pageSize, String GroupInfo) {
		return group.resultMessage(
				group.page(idx, pageSize, JSONHelper.string2json(GroupInfo)));
	}
